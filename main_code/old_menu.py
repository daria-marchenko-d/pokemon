        ...
        # while True:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    pygame.quit()
                    sys.exit()
                if event.type == pygame.MOUSEBUTTONDOWN: # Обробка натискання кнопки "Назад"
                    if self.graphic.button_rect(520, "Go back").colliderect(event.pos): # Перевірка натискання на кнопку "Назад"
                        return # Вихід з menu_game() та повернення в show_main_menu()
